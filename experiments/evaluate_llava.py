...
class LLAVA(nn.Module):
    def __init__(self, args, device_map=None):
        ...
        self.device = 'cuda:{}'.format(args.gpu_id)
        self.device_map = self.create_device_map(32, 4)
...
def main(
    ...
    for case in chunks(ds_eval, 1):
        ...
        metrics = {
            ...
            #
            # "paraphrase_prompts": generate_fast(edited_model, tok, img_embs, record["paraphrase_prompts_qwen3"], 50),
            # "neighborhood_prompts": generate_fast(edited_model, tok, img_embs, record["neighborhood_prompts_qwen3"], 50),
        }
    ...
    if not os.path.exists("results"):
        os.mkdir("results")
    if privacy_scale:
        with open(os.path.join("results", model.model_name + alg_name + f"{f'_{repetition}' if repetition is not None else ''}" + f"{f'_{questions_per_img}' if alg_name == 'Ours' else ''}" + f"{f'_{hparams.kl_factor}' if alg_name != 'In-context' else ''}" + "_largedset_" + f"_{args.privacy_value}_{'single' if answer else 'multi'}{f'_{topk}' if topk else ''}_ans_edited_{privacy_scale}_{dataset_size_limit}_{num_edits}"
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--alg_name",
        choices=["MEMIT", "ROME", "FT", "MEND", "Ours", "In-context", "alphaedit"],
        default="Ours",
    ...
    for alg, la in [("Ours", 11), ("Ours", [9, 10, 11, 12, 13, 14])]:
        main(
            alg,
            args.model_name,
            args.hparams_fname,
