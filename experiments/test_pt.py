...
def main(
    ...
    # state_dict = torch.load("SKU/llava_unlearned_danger/minigpt_org_memflex.pt",  map_location="cpu")
    # state_dict = torch.load("SKU/llava_unlearned_benign/minigpt4-llama2-7b_org_benign.pt", map_location="cpu")
    ...
    if privacy_scale:
        with open(os.path.join("results",
                               model.model_name + alg_name + f"{f'_{repetition}' if repetition is not None else ''}" + "_with_tmps" + f"{f'_{questions_per_img}' if alg_name == 'Ours_bp_cat' else ''}" + f"{f'_{hparams.kl_factor}' if alg_name != 'In-context' else ''}" + "_largedset_" + f"_{args.privacy_value}_{'single' if answer else 'multi'}{f'_{topk}' if topk else ''}_ans_edited_{privacy_scale}_{dataset_size_limit}_{num_edits}"
    ...
    if mme:
        ...
        # with open(os.path.join("results", "MLLMGuard_minigpt_before.json"), "w") as f:
        #     json.dump(mllmguard_qa_res, f, indent=1)
