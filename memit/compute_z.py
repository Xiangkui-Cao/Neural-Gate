...
from memit_main_minigpt4 import generate_fast
...
def compute_z(
    ...
    rewriting_prompts, kl_prompts = [
        ...
        for context_types in context_templates
        for context in context_types
    ], [request["neighborhood_prompts"][0].replace(request["subject"], "{}", 1)]

    kl_targets = generate_fast(model, tok, )
    ...
    for it in range(hparams.v_num_grad_steps):
        ...
        with nethook.TraceDict(
            ...
            kl_logits = torch.stack(
                [
                    logits[i - len(kl_prompts), input_tok["attention_mask"][i - len(kl_prompts)].sum(), :]
                    for i, idx in enumerate(lookup_idxs[-len(kl_prompts):])
                ],
                dim=0,
            )
