...
def layer_stats(
    ...
    def get_ds():
        ...
        # )
        raw_ds = load_dataset("wikitext-103-raw-v1")
        maxlen = 500
        if batch_tokens is not None and batch_tokens < maxlen:
            maxlen = batch_tokens
        return TokenizedDataset(raw_ds["train"], model, tokenizer, maxlen=maxlen)
    ...
    dataloader = DataLoader(dataset=ds, batch_size=batch_size, shuffle=False, num_workers=2, pin_memory=True, collate_fn=lambda x: x)
    with torch.no_grad():
        # for batch_group in progress(loader, total=batch_count):

        for batch_id, batch_group in enumerate(tqdm(dataloader, ncols=100)):
            for batch in batch_group:
                # batch = dict_to_(batch, model.device)
                batch = batch.to(model.device)
                attns = torch.ones_like(batch)
                if model.model_name == "llava1.5-7b":
                    batch = model.model.get_model().embed_tokens(batch).detach()
                else:
                    batch = model.model.embed_tokens(batch).detach()
                with Trace(
                    model.model, layer_name, retain_input=True, retain_output=False, stop=True
                ) as tr:
                    model_outputs(
                        model,
                        inputs_embeds=batch,
                ...
                feats = flatten_masked_batch(tr.input, attns)
